...
public:
    ...
    void deallocate(std::byte* ptr){
        ...
        std::printf("block count -> %lu, bytes for align -> %lu\n", block_pos->count, block_pos->bytes_for_align);
...
arena<2048> global_memory_buffer;
...
public:
    ...
    std::byte* allocate(std::size_t count, std::size_t align = alignof(std::max_align_t)){
        ...
        std::byte* ptr {global_memory_buffer.allocate(count, align)};
        if(ptr != nullptr)
            return ptr;
        ptr = global_memory_buffer.allocate(count, align);
        if(ptr != nullptr)
            return ptr;
        throw std::bad_alloc();
    }
    ...
    void deallocate(std::byte* ptr){
        global_memory_buffer.deallocate(ptr);
    }

    std::size_t get_available_bytes() const {
        return global_memory_buffer.get_available_bytes();
    }

    std::size_t get_occupied_bytes() const {
        return global_memory_buffer.get_occupied_bytes();
    }

    void print_align(){
        global_memory_buffer.print();
    }
